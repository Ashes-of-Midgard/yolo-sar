...
if __name__=="__main__":
    ...
    # model = YOLO("yolov8x-obb-wtconv.yaml", task="obb")
    ...
    # 用自定义的SAR数据集训练/微调
    # results = model.train(data="/root/yolo-sar/datasets/SRSDD-competition-aug-add/dataset.yaml", epochs=300, amp=True, batch=16, device=[0,1,2,3]) # 不建议开启混合精度，非常容易出现数据溢出

    # 评测模型的效果
    results = model.val(data="/root/yolo-sar/datasets/SRSDD-competition-aug-add/dataset.yaml", save_poor = True, batch=1)
